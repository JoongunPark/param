...
class TraceLinker:
    ...
    def __init__(self, pytorch_et_file: str, kineto_file: str, rank: int,
    ...
    def categorize_and_track_kineto_ops(self, kineto_ops: List[KinetoOperator]) -> None:
        ...
        for op in kineto_ops:
            ...
            elif op.is_valid("cuda_runtime") and\
                    op.name in ["cudaLaunchKernel", "cudaMemcpyAsync","cudaStreamSynchronize"]:
    ...
    def load_critical_path_analyzer(self) -> None:
        ...
        """
        self.event_sync_trace = TraceAnalysis(trace_dir = os.path.dirname(self.kineto_file))
        ...
        self.raw_events = self.event_sync_trace.t.get_raw_trace_for_one_rank(rank=self.rank)["traceEvents"]
        for e in self.cp_graph.critical_path_edges_set:
            if e.type in [CPEdgeType.SYNC_DEPENDENCY]:
                ...
                start_ev, end_ev = self.raw_events[start_ev_id], self.raw_events[end_ev_id]
                if 'Ev Idx' in end_ev['args'] and 'Ev Idx' in start_ev['args']:
                    self.sync_deps.setdefault(end_ev['args']['Ev Idx'], []).append(start_ev['args']['Ev Idx'])
                else:
                    self.logger.warning(f"Synchonization dependency starts from event = {start_ev_id} to "
        ...
        if kineto_op.sync_dep:
            ...
            for e in kineto_op.sync_dep:
                ...
                if sync_dep_kineto_op.pytorch_op:
                    ...
                    self.logger.debug(f"Added sync dependency : {pytorch_op.id} -> "
    ...
    def process_op_and_dependents(self, op: Dict) -> List[Dict]:
        ...
        if orig_op_id in self.pytorch_op_id_to_inclusive_dur_map:
            ...
            if orig_op_id in self.pytorch_op_id_to_sync_dep_map:
                op["sync_dep"] = [self.id_assigner.lookup_new_id(dep) for dep in
                        self.pytorch_op_id_to_sync_dep_map[orig_op_id]]
            else:
                op["sync_dep"] = []
...
def main() -> None:
    ...
    parser.add_argument("--rank", type=int, required=True,
                        help="Rank for the input Kineto trace")
